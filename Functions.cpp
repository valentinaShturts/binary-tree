...
void PopularWord(const Tree& tree)
{
    system("cls");
    Node* temp = nullptr;
    tree.PopularWord(tree.GetRoot(), temp);

    if (temp != nullptr)
    {
        cout << "Word of the day:  " << temp->GetEnglish() << " -> " << temp->GetRussian()  << endl;
    }


    temp = nullptr;
    tree.UnpopularWord(tree.GetRoot(), temp);

    if (temp != nullptr)
    {
        cout << "Unpopular word:  " << temp->GetEnglish() << " -> " << temp->GetRussian() << endl;
    }
    _getch();
}