...
Node* Tree::Search(Node* node, const char* key) const
{
    ...
    if (node != nullptr) node->s_counter++;
    return node;
}

Node* Tree::PopularWord(const Node* node, Node*& temp) const
{
    if (node == nullptr) return temp;

    if (temp == nullptr || node->s_counter > temp->s_counter)
    {
        temp = const_cast<Node*>(node);
    }

    PopularWord(node->m_left, temp);
    PopularWord(node->m_right, temp);

    return temp;
}

Node* Tree::UnpopularWord(const Node* node, Node*& temp) const
{
    if (node == nullptr) return temp;
    ...
    UnpopularWord(node->m_left, temp);
    UnpopularWord(node->m_right, temp);
